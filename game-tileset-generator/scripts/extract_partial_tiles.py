...
from partial_alpha import remove_chroma_key


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Extract and normalize tiles from a partial atlas candidate."
    )
...
def crop_object_region(image: Image.Image, fit, origin: dict, span: dict) -> Image.Image:
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path, Path]:
    ...
    missing = [name for name, value in (("--spec", args.spec), ("--input", args.input), ("--tiles-dir", args.tiles_dir), ("--manifest-output", args.manifest_output)) if not value]
    if missing:
        raise SystemExit(f"missing required arguments without --run-dir: {', '.join(missing)}")

    return (Path(args.spec).expanduser().resolve(), Path(args.input).expanduser().resolve(), Path(args.tiles_dir).expanduser().resolve(), Path(args.manifest_output).expanduser().resolve())
...
def main() -> None:
    ...
    if spec["sheet"]["layout_style"] != "partial":
        raise SystemExit("extract_partial_tiles.py only supports sheet.layout_style == 'partial'")
    ...
    for obj in spec["objects"]:
        ...
        keyed = remove_chroma_key(region)
        normalized = normalize_region(keyed, tile_size, obj["span"])
        ...
        manifest_tiles.append(
            {
                ...
                "normalized_size_px": {"width": normalized.width, "height": normalized.height},
    ...
    manifest = {
        ...
        "fitted_crop_box": {"left": fit.crop_box[0], "top": fit.crop_box[1], "right": fit.crop_box[2], "bottom": fit.crop_box[3]},
        "geometry_check": {
