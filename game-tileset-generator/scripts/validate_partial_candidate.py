...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate partial-atlas candidate geometry against spec before extraction."
    )
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path]:
    ...
    missing = [name for name, value in (("--spec", args.spec), ("--input", args.input), ("--output", args.output)) if not value]
    if missing:
        raise SystemExit(f"missing required arguments without --run-dir: {', '.join(missing)}")

    return (Path(args.spec).expanduser().resolve(), Path(args.input).expanduser().resolve(), Path(args.output).expanduser().resolve())
...
def main() -> None:
    ...
    if spec["sheet"]["layout_style"] != "partial":
        raise SystemExit("validate_partial_candidate.py only supports sheet.layout_style == 'partial'")
