...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compose a final normalized partial atlas from extracted tiles."
    )
...
def main() -> None:
    ...
    if spec["sheet"]["layout_style"] != "partial":
        raise SystemExit("compose_partial_atlas.py only supports sheet.layout_style == 'partial'")

    sheet = spec["sheet"]
    tile_size = sheet["tile_size"]
    atlas = Image.new("RGBA", (sheet["columns"] * tile_size, sheet["rows"] * tile_size), (0, 0, 0, 0))
    ...
    for obj in spec["objects"]:
        ...
        atlas.alpha_composite(tile, (obj["origin"]["x"] * tile_size, obj["origin"]["y"] * tile_size))
